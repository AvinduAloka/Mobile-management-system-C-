...
namespace MobilMS
{
    public partial class DashBoard : Form
    {
        ...
        SqlConnection con = new SqlConnection("Data Source=DESKTOP-6S94ROH;Initial Catalog=MMS;Integrated Security=True");
        ...
        private void btnSupplierAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    SqlCommand cmd3 = new SqlCommand("Select SupplierTB.SupplierID, SupplierTB.CompanyName, SupplierTB.ContactMobile, SupplierTB.ContactLand, SupplierTB.AddressNo, SupplierTB.AdddressLine, SupplierTB.AddressCity from SupplierTB order by 1 asc", con);
                    SqlDataAdapter da = new SqlDataAdapter(cmd3);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
                    datagridSupplierList.DataSource = dt;
        ...
        {
            con.Open();
            SqlCommand cmd3 = new SqlCommand("Select ProductTB.PID, ProductTB.Name, ProductTB.PBrand, ProductTB.SName, ProductTB.SN, ProductTB.Units, ProductTB.UnitPrice from ProductTB order by 1 asc", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd3);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        private void btnProductAdd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("insert into ProductTB values(@ID,@PID, @Name, @PBrand,@SName,@SN,@Units,@UnitPrice)", con);
        ...
        private void btnProductUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("update ProductTB set Name= @Name, PBrand= @PBrand, SName= @SName, SN= @SN, Units= @Units, UnitPrice= @UnitPrice where PID= '" + txtProductProductID.Text + "'", con);
                cmd.Parameters.AddWithValue("@PID", txtProductProductID.Text);
                cmd.Parameters.AddWithValue("@Name", txtProductname.Text);
                cmd.Parameters.AddWithValue("@PBrand", cmbProductSeries.SelectedItem);
                cmd.Parameters.AddWithValue("@SName", cmbProductSuppilerName.SelectedItem);
                cmd.Parameters.AddWithValue("@SN", txtProductSN.Text);
                cmd.Parameters.AddWithValue("@Units", int.Parse(txtProductUnits.Text));
                cmd.Parameters.AddWithValue("@UnitPrice", float.Parse(txtProductUnitPrice.Text));
                cmd.ExecuteNonQuery();
        ...
        private void btnProductDelete_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand("DELETE FROM ProductTB WHERE SupplierID = '" + txtProductProductID.Text + "'", con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Delete Successfull!");
        ...
        private void btnListProductadd_Click(object sender, EventArgs e)
        {
            ...
            if (product1 == cmbCustomerSeries.Text)
            {
                ...
                while (srd1.Read())
                {
                    UnitPrice = srd1.GetValue(7).ToString();
                }
                con.Close();
